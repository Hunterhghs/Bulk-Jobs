...
// =============================================================================
// Auto-migration — creates tables on first run if they don't exist
// =============================================================================
...
async function ensureSchema(env: Env): Promise<void> {
  ...
  schemaInitialized = true; // Only attempt once per isolate

  const stmts = [
    `CREATE TABLE IF NOT EXISTS jobs (id TEXT PRIMARY KEY, title TEXT NOT NULL, company TEXT NOT NULL, company_id TEXT NOT NULL, location TEXT DEFAULT 'Remote', description TEXT, description_html TEXT, url TEXT NOT NULL UNIQUE, apply_url TEXT, posted_date TEXT, fetched_at TEXT NOT NULL DEFAULT (datetime('now')), source TEXT NOT NULL, category TEXT, job_type TEXT, level TEXT, region TEXT, relevance_score REAL DEFAULT 0.0, is_new INTEGER DEFAULT 1, is_active INTEGER DEFAULT 1, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    ...
    `CREATE TABLE IF NOT EXISTS companies (id TEXT PRIMARY KEY, name TEXT NOT NULL, ats TEXT NOT NULL, board_token TEXT NOT NULL, category TEXT, region TEXT, is_active INTEGER DEFAULT 1, last_fetched_at TEXT, job_count INTEGER DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')))`,
    `CREATE INDEX IF NOT EXISTS idx_companies_ats ON companies(ats)`,
    `CREATE INDEX IF NOT EXISTS idx_companies_active ON companies(is_active)`,
    `CREATE TABLE IF NOT EXISTS fetch_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, started_at TEXT NOT NULL DEFAULT (datetime('now')), finished_at TEXT, source TEXT, companies_fetched INTEGER DEFAULT 0, jobs_found INTEGER DEFAULT 0, jobs_new INTEGER DEFAULT 0, jobs_updated INTEGER DEFAULT 0, errors TEXT, status TEXT DEFAULT 'running')`,
  ...
  console.log('[Bulk Jobs] Schema ensured');
}
